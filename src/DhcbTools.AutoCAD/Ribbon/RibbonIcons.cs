...
/// Bảng màu theo nhóm chức năng để mắt nhận ra panel trước khi kịp đọc chữ.
/// </summary>
internal static class RibbonIcons
{
    ...
    public static ImageSource Create(string glyph, Color color)
    {
        ...
        {
            // Chừa lề: icon vẽ kín khung trông nặng và to hơn hẳn icon gốc của Autodesk bên cạnh.
            var margin = size * 0.16;
            var box = size - (margin * 2);
            var rect = new Rect(margin, margin, box, box);
            var radius = box * 0.26;

            // Nền chuyển sắc nhẹ: phẳng nhưng không bẹt, hợp giao diện AutoCAD/Revit hiện tại.
