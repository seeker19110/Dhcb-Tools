...
namespace DhcbTools.Shared.Logic.Ai
{
    ...
    public static class CommandCatalog
    {
        ...
        public static readonly IReadOnlyList<CommandDescriptor> All = new List<CommandDescriptor>
        {
            ...
            // ── Revit — MEPF ────────────────────────────────────────────────
            new CommandDescriptor("FamilyUpgrade", Revit, "Nâng cấp hàng loạt file .rfa sang định dạng của phiên bản Revit đang chạy, ghi sang thư mục khác — cách duy nhất có thư viện family dùng được cho nhiều phiên bản Revit", true, "NangCapFamily", "FamilyMultiVersion")
                .Field("sourceFolder", "thư mục chứa .rfa cần nâng cấp (chỉ đọc)").Field("outputFolder", "thư mục ghi bản đã nâng cấp — phải nằm ngoài thư mục nguồn")
                .Field("recursive", "quét cả thư mục con (mặc định true)").Field("overwrite", "ghi đè file đã có ở đích")
                .Field("maxFiles", "tối đa ngần này file (0 = tất cả)", FieldKind.Number).Field("dryRun", "xem trước")
                .Words("nâng cấp family", "upgrade family", "family nhiều phiên bản"),
            ...
            new CommandDescriptor("ConnectorChecker", Revit, "Liệt kê connector MEP hở", true, "ConnectorCheck", "CheckConnectors")
                ...
                .Field("outputPath", "CSV từng connector hở: Key, ElementId, Category, Level, Domain, Shape, X/Y/Z mm (rỗng = chỉ Messages)")
                .Field("acceptedPath", "JSON đã chấp nhận (cùng định dạng clash-accepted.json, khoá = cột Key của CSV) — bỏ qua và đếm riêng")
            ...
            new CommandDescriptor("ConstructionStatus", Revit, "Ghi trạng thái thi công (chưa lắp / đang lắp / đã lắp / đã nghiệm thu) từ CSV hiện trường vào mô hình", true, "TrangThaiThiCong", "InstallStatus")
                ...
                .Field("dateFormat", "dạng ngày ghi vào tham số Text, mặc định dd/MM/yyyy")
            ...
            new CommandDescriptor("ProgressReport", Revit, "Báo cáo tiến độ thi công: % theo số lượng và theo chiều dài, gộp theo tầng/hệ/category, luỹ kế theo tuần → HTML + CSV", false, "TienDo", "Progress")
                ...
                .Field("statusParameter", "tham số trạng thái (rỗng = từ điển constructionStatus)")
                .Field("dateParameter", "tham số ngày cho chuỗi theo tuần (rỗng = từ điển constructionDate)")
                .Field("levelName", "chỉ tầng này").Field("systemContains", "lọc hệ chứa chuỗi")
