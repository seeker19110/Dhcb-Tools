...
- **`AutoRoute` — chất lượng tuyến mới đo trên một model mẫu.** Từ §55 mỗi tuyến báo *dài/Manhattan* và *số rẽ so với tối
  ...
  sàn rồi quay lại — đúng hình học model). Trên dự án A (§57): một tuyến 7,9 m giữa hai đầu duct = 1,00×, 1 rẽ — mới một tuyến. Lịch sử: phần đọc model liên kết đã sửa (§18: vật cản
