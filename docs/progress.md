...
>
> **Còn lại:** 9.4 — người thật dùng hai tuần (vòng đóng vai sáu kỹ sư §57 trả lời được "bấm có chạy không, vướng
> đâu", không trả lời được "có dùng tiếp không"). `AutoRoute` đã có số trên dự án thật (7,9 m = 1,00×, §57) nhưng mới một tuyến.
...
**Revit (43):** ParameterExport, ParameterImport, RemoveUnusedViews, AutoNumbering, BatchExport, HealthReport,
ProjectInfo, LevelSetup, GridSetup, FamilyLoader, FamilyStarter, FamilyUpgrade, SleeveAuto, ElevationTag, HangerAuto, PipeSplitter, ConnectorChecker,
...
| **11.4 — xác nhận thật đường Revit của `relation`** | 2026-09-07 | Câu hỏi để ngỏ ở §71: đường Revit (`Element.LevelId`) có cùng lỗi "phần của tổ hợp không thừa vị trí" không? Chạy thật Revit 2024 trên Snowdon Architectural: cả 142 cửa, kể cả 7 cửa curtain wall mà đường IFC từng báo nhầm, đều đạt `partOf relation="IFCRELCONTAINEDINSPATIALSTRUCTURE"` — không có lỗi tương tự, vì `LevelId` gán trực tiếp không qua suy luận cha. Không có thay đổi mã nguồn, chỉ xác nhận bằng chạy thật — §72. Đóng nốt mục 11.4 của roadmap.md — không còn việc treo
| **11.6 — `AsBuiltStamp` khép mục 11.6** | 2026-09-07 | Không dựng family (Revit API không có `NewLabel` — kiểm bằng phản chiếu toàn bộ `RevitAPI.dll`): vẽ thẳng `DetailCurve`/`TextNote` lên sheet theo đúng khuôn Phụ lục IIb đọc từ văn bản gốc (PDF ký số, quét ảnh). Hai lỗi lộ ra khi chạy thật: `OfClass(DetailLine)` ném ArgumentException (Revit tự chỉ cách sửa), và chống trùng bằng tham số Comments thất bại vì tham số đó không tồn tại trên category Text Notes/Lines của model — đổi sang nhận diện bằng vùng hình học của khuôn dấu. `revit-write` 19/19 (vẽ 57/57 → lặp lại 0/57 vì đã có → overwrite vẽ lại 57/57), `revit-smoke` 43/43+1. Shared.Logic 1651, phủ 100% — §73
...
| **Quét sâu lần hai: 3 chỗ nói dối về mô hình** | 2026-09-06 | Phân loại 34 khối `catch` rỗng còn lại theo tiêu chí §61 — 28 khối cố ý giữ nguyên, 5 khối sửa. `ElevationUpdater` thu hẹp `try` vào trong vòng lặp (trước: một phần tử hỏng là cả lượt bỏ dở, không nhật ký); `ModelChoices` trả `ChoiceList` mang cả cận dưới lẫn lý do dừng nên form hết ghi "mô hình chưa có giá trị nào" khi thực ra là đọc hỏng; `SleeveAuto` báo khi lùi về lọc hộp bao. **5 ca mới**, Shared.Logic 1612 → 1617, 100 %; 9 cảnh báo build → 0. Revit 2024.3: `smoke` 41/41+1, `mep` 29/29; Revit 2026: `mep` 29/29 + `smoke` 41/41+1 (mọi con số trùng, trừ các chỗ do model mẫu 2026 khác — đã biết từ §51/§59). `plumbing` 8/8 và ba bộ ghi thật 16/16 + 21/21 + 5/5 trên cả hai (435/552/455/562 sleeve, 1.120/1.125 hanger trùng khít). `autoroute` 13/13 trùng nguyên từng ô lưới và số node. Cộng **bảy bộ × hai phiên bản: 266 ca chạy thật, 0 trượt** — toàn bộ bộ ca kiểm Revit đã chạy lại; thêm `autocad-smoke` 18/18 (AcadScriptGen vẫn sinh script `.scr` chạy được) → **284 ca** — §64 |
| **`FamilyUpgrade` — thư viện family cho nhiều phiên bản Revit** | 2026-09-06 | Lệnh Core mới mở hàng loạt `.rfa` rồi lưu sang thư mục khác theo định dạng của Revit ĐANG CHẠY; `outputFolder` bắt buộc nằm ngoài `sourceFolder` nên bản gốc không bao giờ bị nâng cấp đè (nâng cấp .rfa là một chiều). Kèm `scripts/dung-family.ps1` chạy trọn vòng build → cài add-in → dựng/nâng cấp cho từng phiên bản. `write-mep` 21/21 trên Revit 2024 và 2026 — §63 |
| **Chấp nhận connector, family nhận tham số, 5 tuyến AutoRoute dự án A** | 2026-09-06 | `ConnectorChecker` có `acceptedPath` + cột Key (37 → 36 + 1 bỏ qua); `FamilyStarter` gắn nhãn `Nominal Width` lên kích thước đường kính (chốt bằng `messagesContain`, `write-mep` 19/19); `AutoRoute` thêm 5 tuyến dự án A — 5/6 tuyến 1,00× Manhattan — §62 |
| **`FamilyStarter` — family mẫu sleeve/hanger** | 2026-09-06 | Lệnh Core mới dựng `DHCB_Sleeve`/`DHCB_Hanger` từ template Generic Model của chính Revit, ghi .rfa, nạp vào mô hình (work-plane-based). `write-mep` 19/19: SleeveAuto với DHCB_Sleeve 455 → 0, HangerAuto 1125; `mep` 28/28. Hình giữ chỗ, chưa tham số hoá hình học — §60 |
| **Năm bộ hôm nay trên Revit 2026** | 2026-09-06 | `mep` 27/27, `plumbing` 8/8, `autoroute` 13/13, `write-plumbing` 5/5, `write-mep` 15/15 — con số trùng 2024.3; id ống của model mẫu giữ nguyên giữa hai phiên bản nên điểm (5) của §54 khép bằng bằng chứng, không đổi mã — §59 |
| **Sửa hai điểm vướng của vai MEP** | 2026-09-06 | `FamilyCandidates`: lỗi thiếu family của `SleeveAuto`/`HangerAuto` liệt kê 8 family có thật (từ khoá → category phụ kiện → instance; fitting và chú thích xuống cuối — hai luật lộ ra từ hai lượt `mep`); `ConnectorChecker` có `outputPath` CSV. `mep` 27/27, `plumbing` 8/8 — §58 |
| **Đóng vai sáu kỹ sư thay nhóm 9.4** | 2026-09-06 | 24 lệnh chạy được ngay, 1 bỏ có lý do; lộ **lỗi câm** của runner AutoCAD (`--plugin-dll` tương đối → NETLOAD hỏng, báo "0 OK, 0 lỗi") — sửa + `NetloadFailure`; `BlockMessages` liệt kê block/tag có thật; `SheetIndex` hết lặp chữ; **AutoRoute lần đầu trên dự án thật: 7,9 m = 1,00× Manhattan**; `don-ket-qua.ps1` dọn journal — §57, [`phan-hoi-dong-vai-2026-09-06.md`](phan-hoi-dong-vai-2026-09-06.md) |
...
| **11.3 — gói bàn giao đêm chạy thật + Task Scheduler tự chạy lần đầu** | 2026-09-05 | Lệnh mới `SheetIndex` (55 sheet Snowdon), tầng thuần `HandoverPackage` (băm SHA-256, ô ký Điều 11 NĐ 207/2026), khối `handover` trong job. Dự án A ARC L01: IFC4 13 MB đạt, IDS 0 lỗi **ở cả hai đường**. Ba lỗi lộ và sửa: BatchExport đòi sheet cả khi xuất IFC; BuildHandover sửa danh sách đang duyệt; Revit-path Tag rỗng khi Mark rỗng (12 lỗi giả). Task đăng ký, Start-ScheduledTask Result 0 — §43 |
| **`DictionaryLearn` trên dự án A — "Join Status" suýt thành trạng thái thi công** | 2026-09-05 | 8 file `_upgraded-2024/` dryRun: `constructionStatus → "Join Status"` (nối dầm Revit) chấm 0,78, sẽ được ghi nếu tắt dryRun rồi `ProgressReport` đọc sai im lặng. Sửa: tên có **từ hạn định** (chứa trọn tên đồng nghĩa + thêm chữ) chặn ở 0,69 → chỉ [Xem]. Chạy lại 8 file: 0 khoá tự ghi. Chuỗi băm log dự án thật nguyên vẹn cả hai lượt — §42 |
...
- **`AutoRoute` — chất lượng tuyến mới đo trên một model mẫu.** Từ §55 mỗi tuyến báo *dài/Manhattan* và *số rẽ so với tối
  ...
  sàn rồi quay lại — đúng hình học model). **Trên dự án A: 6 tuyến (§57, §62) — 5 tuyến 1,00×, 1 tuyến 1,19× vì né 2 vật cản.** Lịch sử: phần đọc model liên kết đã sửa (§18: vật cản
...
2. ~~Một đêm batch thật trên **dự án thật**~~ — phần "chạy được, chạy đúng, không làm hỏng gì trên dữ
   ...
   ✅ 2026-09-05 (§43): job gói bàn giao dự án A **đã đăng ký và tự chạy qua `install-nightly-task.ps1`**, Result 0, 23:50 hằng đêm. Trước đó: chưa có job nào thật sự chạy tự động — hai lượt ở §20 đều
...
4. ~~Gỡ ma sát từ điển tham số phát hiện ở §21~~ — xong: lệnh **`DictionaryLearn`** soi tên tham số thật
   ...
   thì dừng chứ không ghi đè). ✅ Chạy thật trên dự án A 2026-09-05 (§42): lộ **"Join Status" được chấm 0,78 cho `constructionStatus`** — tên có từ hạn định nay bị chặn ở mức "kỹ sư xem" (`HasQualifier`), chạy lại 8 file 0 khoá tự ghi. Dự án A không có khoá thi công nào thật → mục 9 (shared parameter) là tiên quyết.
...
7. Rồi tới **9.4 — đưa cho một nhóm kỹ sư dùng thật**; phản hồi của họ quyết định giai đoạn 10/11 đi sâu
   ...
   *dùng hằng tuần / bấm rồi bỏ / chưa dùng* cho đủ 49 lệnh Revit + 15 lệnh AutoCAD, kèm bốn câu hỏi mở.
   ...
   ✅ **v1.1.0 đã phát hành 2026-09-06** (§45) và có trang [`phat-hanh-v1.1.md`](phat-hanh-v1.1.md) với nhóm 6 vai đề xuất; còn chọn tên người và thu phản hồi sau 2/4 tuần — việc của người.
8. ~~Chuỗi băm cho nhật ký batch (11.5)~~ — xong, **đã chạy thật cả hai đường (§24)**: đêm batch Revit
   ...
   chạy trên CI. ✅ Log **dự án thật** đã kiểm 2026-09-05 (§42): 8 dòng, có dòng lỗi xen giữa, tên file có dấu — nguyên vẹn. ⬜ Còn: chỉ số 30 ngày (theo định nghĩa phải chờ 30 ngày) và log của **dự án thật** như
...
10. ~~**B3 — BCF cho `ClashDetection`**~~ — xong 2026-09-05: khai `bcfPath` thì lệnh ghi thêm file **BCF 2.1**
   mở trong Solibri/BIMcollab, hoặc Navisworks có cài BCF Manager (Navisworks không đọc BCF nguyên bản), mỗi va chạm một topic có camera nhìn vào tâm. Làm được ngay,
   ...
   ⬜ Còn: **mở thử trong Solibri/BIMcollab hoặc Navisworks đã cài BCF Manager** — máy kiểm 2026-09-11 có
   Navisworks Manage 2026 nhưng không có plugin BCF, nên không thể xác minh import bằng bản cài nguyên bản.
