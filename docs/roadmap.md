...
| 9.3 🟡 | **Tiếng Việt hoá thông báo** | ✅ Đã dọn các chuỗi tiếng Anh còn sót mà vòng chạy thật 2026-09-03 in ra tận báo cáo: `[Dry Run] Would create 2 level(s).`, `[Skip]`, `[Create]`, `Family folder not found`, `Error: `, `[DryRun]` của BatchExport. `VietnameseMessageTests` (thuần, đọc mã nguồn Core) chốt chặn theo danh sách mẫu đã thấy tận mắt. ✅ Thêm mã lỗi `E-CONFIG-MISSING` (thiếu trường bắt buộc) cạnh `E-PARAM-MISSING`/`E-PATH-MISSING` sẵn có. ✅ **Bảng mã lỗi** gom vào [`ma-loi.md`](ma-loi.md) — 4 mã (`E-CONFIG-MISSING`, `E-PATH-MISSING`, `E-PARAM-MISSING`, `E-PARAM-READONLY`), mỗi mã kèm nghĩa/khi nào gặp/cách xử lý; `MaLoiTests` đối chiếu tài liệu với mã nguồn **cả hai chiều** nên bảng không trôi. ⬜ Còn: rà nốt phần `Messages` chi tiết của nhóm MEPF |
