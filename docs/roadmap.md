...
| 8.4 🟡 | **Vòng kiểm thử thật số 1 trọn 42 lệnh** trên Revit 2024.3 + AutoCAD 2026 | Hai bộ ca kiểm tự động đã sẵn sàng (smoke + MEP, dùng model mẫu kèm Revit) và chạy bằng một lệnh: `scripts/run-in-revit-tests.ps1`. **Chưa chạy được vòng đầu vì Revit đang mở lúc dựng xong** — script tự dừng khi thấy Revit chạy. Phần còn lại vẫn theo [`huong-dan-cai-dat-va-kiem-thu-thu-cong.md`](huong-dan-cai-dat-va-kiem-thu-thu-cong.md) R1–R48, C1–C17, B1–B12; ghi vào [`bang-chung-test.md`](bang-chung-test.md) §6, mỗi lỗi kèm test tái hiện phần thuần | 1 tuần |
...
| 10.4 ✅ | **Đóng gói `.mcpb`** cho Claude Desktop | ✅ [`tools/mcpb/manifest.json`](../tools/mcpb/manifest.json) + [`scripts/pack-mcpb.ps1`](../scripts/pack-mcpb.ps1) → `dist/dhcb-<app>-<phiên bản>.mcpb`, mở bằng Claude Desktop là xong. Token để trống thì tự đọc `bridge-token.txt`. **Đã đóng gói thật**: 9,1 KB, 4 file, không dependency ngoài. ⬜ MCP server chịu được khi Revit chưa mở (hiện `tools/list` báo lỗi kết nối) |
