...
assigns-to-group) vì không có đối tượng quan hệ IFC thật để đọc. Test thuần trên CI (`IdsTests`, `IfcIdsElementTests`);
**chưa đối chiếu lại với IfcTester cho riêng thuộc tính `relation`** (bản đối chiếu 10/10 ở trên chạy trước khi có
sửa này) và chưa chạy thật trong Revit — hai việc đó vẫn mở. Solibri chưa có trên máy. Bằng chứng §41 |
