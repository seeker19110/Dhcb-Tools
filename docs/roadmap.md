...
| B3 | **BCF 2.1** gửi tư vấn — `bcfPath` trên `ClashDetection`, `ParameterRuleCheck`, `WarningsExport` | ✅ mã nguồn 2026-09-05: tầng thuần `Shared.Logic/Bcf` (19 ca test, đọc lại chính file vừa ghi), IFC GUID lấy bằng `ExportUtils.GetExportId` nên khớp file IFC đã nộp. **Không thêm lệnh Core** — đầu ra thêm của ba lệnh đã chạy thật, giữ nguyên tắc 6 như tiền lệ `--verify-log`. 🧪 Chưa mở lại bằng máy đọc BCF thật. Tài liệu: [`bcf.md`](bcf.md) |
| C4 | `ModelLinesFromCad` — dựng model line từ DWG, nối `CadLayerMap` với `RouteFromLines` | ⬜ |
