...
Đề xuất CAD → BIM nâng cao chưa triển khai được khóa trước ở
[`dac-ta-cad-to-bim-nang-cao.md`](dac-ta-cad-to-bim-nang-cao.md) và
[`dac-ta-trien-khai-cad-to-bim-nang-cao.md`](dac-ta-trien-khai-cad-to-bim-nang-cao.md). Hai tài liệu này không
thay quyết định dừng mở rộng bề mặt lệnh bên dưới: chỉ bắt đầu từng lát khi có fixture và host test tương ứng.
...
| 9.4 ✅ (đã phát hành, chờ người) | **Đưa cho một nhóm kỹ sư dùng thật** | ✅ **v1.1.0 phát hành 2026-09-06** (installer + 7 zip), cài thật bằng installer, trang phát hành và nhóm 6 vai đề xuất ở [`phat-hanh-v1.1.md`](phat-hanh-v1.1.md); còn chọn tên người và thu phản hồi sau 2/4 tuần (§45). Mô tả gốc:  Phát hành v1.1, thu phản hồi theo mẫu (lệnh nào dùng hằng tuần, lệnh nào bấm rồi bỏ). Số liệu này quyết định giai đoạn 10/11 đi sâu vào đâu. ✅ **Mẫu thu phản hồi**: [`mau-phan-hoi-9-4.md`](mau-phan-hoi-9-4.md) — bảng tick *tuần / bỏ / chưa* cho đủ 42 lệnh Revit + 15 lệnh AutoCAD, cột lý do bắt buộc khi tick *bỏ*, bốn câu hỏi mở, và cách tổng hợp theo lệnh chứ không theo người; `PhanHoiFormTests` đối chiếu hai chiều với `CommandCatalog` nên mẫu không trôi khi thêm/bớt lệnh. ✅ v1.1.1 đã phát hành 2026-09-05; ⬜ chọn nhóm kỹ sư — vòng **đóng vai 6 kỹ sư** thay người thật đã làm 2026-09-06 (§57, [`phan-hoi-dong-vai-2026-09-06.md`](phan-hoi-dong-vai-2026-09-06.md): 24 lệnh chạy ngay, 1 bỏ, 3 lỗi sửa); còn thiếu câu "có dùng tiếp không" chỉ người thật trả lời được |
...
| # | Việc | Chi tiết |
|---|---|---|
| 11.1 ✅ | **Checkset theo giai đoạn**, nền là **IDS** chứ không phải JSON tự nghĩ | ✅ 2026-09-05. Đọc file **IDS 1.0** (buildingSMART, chuẩn chính thức từ 01/6/2024): chủ đầu tư/tư vấn thẩm tra khai yêu cầu **một lần**, kiểm được bằng cả DHCB lẫn IfcTester/Solibri và — theo đúng mục đích của IDS — phải nhận **cùng một kết quả**. Kiểm **thẳng trên mô hình Revit**, không vòng qua IFC: kỹ sư sửa ngay tại chỗ. Tầng thuần `Shared.Logic/Ids` (`IdsSpec` đọc XML + `IdsEvaluator` đánh giá, đủ 6 loại facet, 34 ca test) — lệnh Core **`IdsValidate`**, nút Ribbon *Kiểm tra & AI → Kiểm theo IDS*, 2 ca kiểm trong `revit-smoke`. Gặp thứ chưa hỗ trợ (facet lạ, ràng buộc `minLength`, file không có `<specification>`) thì **từ chối file** chứ không bỏ qua im lặng: một quy tắc bị lờ đi vẫn in ra dấu ✓ mà người đọc không có cách nào biết. ✅ **Chạy thật** trên Revit 2024: *1270 phần tử, 3 specification, 42 không đạt, 1 specification không có phần tử nào để kiểm* — nhóm thứ ba được đánh dấu và đếm riêng, vì "0 không đạt" ở đó nói về bộ lọc chứ không nói về mô hình (bài học §16). ⚠️ **Ranh giới**: DHCB đọc mô hình theo ánh xạ Revit → IFC (`IfcExportAs`, category, tham số đóng vai property) — cùng ánh xạ bộ xuất IFC dùng, nhưng **không phải chính file IFC**, nên kết luận là "mô hình sẽ đạt khi xuất". ⬜ Còn: **đối chiếu với IfcTester/Solibri** trên cùng một file IDS — câu "ba phần mềm cùng kết luận" mới là mục đích của IDS, chưa phải điều đã chứng minh. `ParameterRuleCheck` và `configs/checksets/` **giữ nguyên** cho quy tắc nội bộ công ty mà IDS không mô tả: đặt tên file/view/sheet theo BEP, worksets, link, ngưỡng warning/dung lượng. Tài liệu: [`kiem-ids.md`](kiem-ids.md). = đề xuất **C3**
...
| 11.3 ✅ (2/2 lượt trên dự án A, xem chú thích §45) | **Gói bàn giao tự động** | ✅ 2026-09-05: khối `handover` trong job → `ban-giao.html` (in được, ô xác nhận chủ đầu tư) + `ban-giao.json`: băm SHA-256 từng file, danh mục bản vẽ từ lệnh mới `SheetIndex`, kiểm chuỗi băm nhật ký / IFC / IDS (đường IFC của 11.4), dấu thời gian, InformationalVersion có SHA commit. Chạy thật dự án A (ARC L01): IFC4 13 MB đạt, IDS 0 lỗi ở cả hai đường sau khi sửa Tag = ElementId như bộ xuất. **Lần đầu Task Scheduler tự chạy** (Result 0, 23:50 hằng đêm). ⬜ Dự án thứ hai cần mô hình có sheet để PDF + danh mục vào gói. Bằng chứng §43. Mô tả gốc: Một job đêm sinh: IFC, PDF, danh mục bản vẽ, báo cáo tuân thủ HTML/PDF có dấu thời gian và version add-in. **Thêm theo Điều 11 NĐ 207/2026:** mọi đầu ra điện tử phải **trích xuất được ra PDF/giấy** kèm chỗ **chủ đầu tư xác nhận** — không được chỉ tồn tại dưới dạng dữ liệu trong máy. Chạy trên 2 dự án thật trước khi công bố |
| 11.4 ✅ | Quyết định **mức sâu của kiểm IDS** | ✅ 2026-09-05: **mở sang file IFC**. `IfcIdsElement` (tầng thuần, trên bộ đọc STEP của 11.2) + `BatchRunner --verify-ifc … --verify-ids`; cùng `IdsEvaluator` và cùng `IdsReport` với đường Revit. Đối chiếu với IfcTester 0.8.5 trên Snowdon: **10/10 specification khớp từng con số** (predefinedType, property boolean/chuỗi, classification theo hệ, material, partOf, prohibited, số, pattern). Lộ và sửa 3 lỗi của DHCB (boolean `.F.`, thuộc tính riêng lớp, số không đạt bị cắt ở 200 — lỗi này có cả ở đường Revit) và 1 lỗi của IfcTester (`FALSE` viết hoa bị coi là True). Lý do mở: rẻ, chạy CI, là thứ duy nhất bắt được lỗi ánh xạ (§39) lẫn lỗi đếm (§41). Đường Revit giữ cho kỹ sư sửa tại chỗ. ✅ **`partOf` nay xét `relation`** (2026-09-06): thuộc tính `relation` của facet `partOf` (`ids.xsd` 1.0) được đọc và
kiểm đúng ngữ nghĩa buildingSMART — khai `relation` thì chỉ chuỗi **thuần một loại quan hệ** đó mới khớp (không
rơi về chuỗi trộn nhiều loại như trước); không khai vẫn như cũ. Cả năm loại quan hệ của lược đồ (kể cả cặp gộp
voids/fills của opening). Đường Revit khớp gần đúng hai loại (tầng ↔ contained-in-spatial-structure, hệ ↔
assigns-to-group) vì không có đối tượng quan hệ IFC thật để đọc. Test thuần trên CI (`IdsTests`, `IfcIdsElementTests`); ✅ **đã chạy thật trong Revit 2024** (2026-09-06):
fixture ba specification tách riêng đúng biến `relation` (bỏ hẳn `<entity>` để không lẫn với khác biệt tên
đã biết) trên cả Snowdon Architectural lẫn HVAC — cửa khớp `IFCRELCONTAINEDINSPATIALSTRUCTURE` 142/142,
đối chứng `IFCRELAGGREGATES` (quan hệ Revit không bao giờ sinh ra) trượt 142/142 đúng như kỳ vọng, duct
khớp `IFCRELASSIGNSTOGROUP` 1053/1053 — chứng minh bộ so `relation` phân biệt được thật, không phải lỗ
hổng "chấp nhận mọi quan hệ". Bằng chứng §69. ✅ **Đã đối chiếu với IfcTester cho riêng `relation`**
(2026-09-07, §71): fixture 13 specification phủ cả năm giá trị enum `relations` của `ids.xsd` chạy trên
chính file IFC Snowdon Architectural — **13/13 khớp từng con số** với IfcTester 0.8.5, trong đó ba ca đối
chứng (quan hệ sai loại, quan hệ mô hình không có) trượt 100% ở cả hai bên. Vòng này lộ ra **lỗi thứ tư**
mà bộ tham chiếu độc lập bắt được: phần của một tổ hợp (7 cửa curtain wall) không thừa vị trí không gian
của tổng nên bị báo nhầm "không thuộc tầng nào" — đã sửa, có test thuần chốt cả hai chiều (thừa đúng cấp
gần nhất, không leo lên toà nhà). ✅ **Đường Revit đã chạy thật, không có lỗi tương tự** (§72): cả 142 cửa
Snowdon, kể cả 7 cửa curtain wall mà đường IFC từng báo nhầm, đều đạt trên `partOf
relation="IFCRELCONTAINEDINSPATIALSTRUCTURE"` — `Element.LevelId` Revit gán trực tiếp cho mọi phần tử kể
cả phần tử lồng trong host khác, nên không có bước suy luận qua cha nào để mà sai. ⬜ Còn: Solibri (thương
mại) chưa có trên máy. Bằng chứng §41, §71, §72 |
...
| 11.6 ✅ | **`AsBuiltStamp` + `DossierIndex`** — dấu hoàn công và danh mục hồ sơ | ✅ 2026-09-06: **`DossierIndex`** (`Shared.Logic/AsBuilt` + `BatchRunner --dossier`) đối chiếu danh mục với file thật, báo thiếu mục bắt buộc, liệt kê file chưa xếp mục, HTML có ô ký Điều 28; danh mục nằm ở **file cấu hình của dự án** vì Phụ lục VII là văn bản pháp luật — DHCB không phát biểu hộ (§48). ✅ **`AsBuiltStamp` xong 2026-09-07** (§73): đọc đúng Phụ lục IIb từ văn bản gốc trên `vanban.chinhphu.vn` (PDF ký số, quét ảnh, đọc bằng mắt qua từng trang) — **không dựng family**: phản chiếu toàn bộ `RevitAPI.dll` cho thấy Revit API công khai không có `NewLabel` (cách duy nhất tạo Label gắn tham số trong family annotation), nên đổi hướng vẽ thẳng `DetailCurve`/`TextNote` lên sheet, cùng kết quả cuối. Lệnh Core mới nhận `mau` (1 hoặc 2 theo đúng khuôn Phụ lục IIb), điền tên nhà thầu/người lập/chỉ huy trưởng/tư vấn giám sát trưởng từ config, chống trùng bằng vùng hình học của khuôn dấu (không dùng tham số Comments — chạy thật lộ ra tham số đó không tồn tại trên category Text Notes/Lines của model mẫu). **Chạy thật Revit 2024**: `revit-write` 19/19 (vẽ 57/57 sheet → chạy lại 0/57 vì đã có → overwrite vẽ lại 57/57), `revit-smoke` 43/43+1 bỏ qua (dry-run + E-CONFIG-MISSING đúng tên trường thiếu). ✅ **Sửa hồi quy hiệu năng 2026-09-11 (#155):** profile tách pha xác định `ViewSheet.Outline` tiêu tốn 190 giây/55 sheet do buộc Revit dựng vùng hiển thị từng sheet; đổi sang dùng trực tiếp gốc `[0,0]` của paper space và lập chỉ mục phần tử một lượt theo `OwnerViewId`, không phụ thuộc điểm chèn family khung tên. Host test: preview **84 ms** (trước sửa 143.794–259.507 ms), `revit-smoke` **43 đạt/0 trượt/1 bỏ qua**; bộ hồi quy ghi riêng `revit-write-asbuilt` **3/3** (vẽ 55/55 → chạy lại 0/55 trong 41 ms → overwrite 55/55). Gán revision "Hoàn công" + xuất PDF vẫn dùng `RevisionOnSheets`/`BatchExport` sẵn có, không làm thêm lệnh. = đề xuất **C2** |
...
| # | Việc | Trạng thái |
|---|---|---|
| A1 | **`SetoutExport`** — toạ độ định vị (tim cột, tâm thiết bị/sleeve, giao trục) ra CSV theo thứ tự cột máy toàn đạc (`PNEZD`/`PENZD`…) + DXF điểm, hệ Survey tự kiểm chiều transform, tên điểm ≤ 16 ký tự không trùng | ✅ mã nguồn 2026-09-05: tầng thuần `Shared.Logic/Setout` + `Geometry/GridIntersections` (50 ca test), lệnh Core, Ribbon, 4 ca kiểm trong `revit-smoke`/`revit-mep`, playbook `skills/xuat-toa-do-dinh-vi`. ✅ **Đã chạy thật 2026-09-05** trên Revit 2024.3: kiến trúc **260 điểm** (118 tim cột + 142 giao trục, CSV + DXF có POINT/TEXT), HVAC **545 điểm** thiết bị, toạ độ Survey thật. Vòng này lộ ra một lỗi: tên giao trục `TrụcA-TrụcB` bị **cắt đuôi** nên mất đúng phần phân biệt (`Block_35_Left-Bl`, `Block_35_Left-B.`) — duy nhất nhưng trắc đạc không đọc được là điểm nào; sửa thành bỏ ở giữa giữ cả đầu lẫn đuôi, hậu tố `_N` vô nghĩa giảm 21 → 10. Bằng chứng: [`bang-chung-test.md`](bang-chung-test.md) §28. Tài liệu: [`toa-do-dinh-vi.md`](toa-do-dinh-vi.md) |
| B1 | **`ConstructionStatus`** + **`ProgressReport`** — trạng thái lắp đặt/nghiệm thu từ CSV hiện trường (từ vựng Việt/Anh, có hay không dấu đều nhận), % theo **số lượng và chiều dài**, gộp theo tầng/hệ/category, luỹ kế theo tuần → HTML + CSV | ✅ mã nguồn 2026-09-05: tầng thuần `Shared.Logic/Progress` (41 ca test), hai lệnh Core, nút Ribbon, 6 ca kiểm, playbook `skills/theo-doi-tien-do-thi-cong`. ✅ **`ProgressReport` đã chạy thật 2026-09-05**: kiến trúc 142 cấu kiện / 9 nhóm theo tầng, HVAC **1599 cấu kiện / 185 nhóm theo hệ** kèm % theo chiều dài (§28). ✅ **Đường ghi của `ConstructionStatus` nay có ca kiểm tự động** (PR #77, 2026-09-05): khai `keyParameter` (ví dụ `Mark`) thì CSV khớp theo mã hiện trường thay vì ElementId của đúng file đang mở, nên fixture nằm được trong repo — bộ `revit-write` có chuỗi 4 ca ghi thật (ghi 3 cửa → ghi lại phải 0 đổi → CSV lùi trạng thái bị chặn → `ProgressReport` ra 1,4%, lần đầu con số tiến độ nói về công trường chứ không phải 0% vì thiếu tham số). Xác nhận lại 2026-09-06: **16/16 đạt** trên Revit 2024 (Snowdon Architectural). Tài liệu: [`tien-do-thi-cong.md`](tien-do-thi-cong.md) |
| B3 | **BCF 2.1 cho `ClashDetection`** — thêm `bcfPath`: mỗi va chạm một topic (tiêu đề, mô tả, nhãn "trong file"/"với model liên kết", camera phối cảnh nhìn vào tâm, hai phần tử liên quan) | ✅ mã nguồn 2026-09-05: tầng thuần `Shared.Logic/Bcf` (21 ca test, đọc lại chính zip vừa ghi), nối vào lệnh đã chạy thật nên **không thêm lệnh Core mới**; GUID topic sinh từ `key` va chạm nên xuất lại không đẻ vấn đề mới. ✅ **Đã chạy thật 2026-09-05** trong `revit-mep`: `clash.bcf` 9.897 byte, **7 topic**, đọc lại bằng thư viện zip thấy đủ `bcf.version` + `project.bcfp` + mỗi topic một `markup.bcf`/`viewpoint.bcfv`, nhãn "Với model liên kết" và tên model liên kết nằm đúng chỗ (§28). ⬜ Còn: mở thử trong Solibri/BIMcollab hoặc Navisworks **đã cài BCF Manager**; xác minh 2026-09-11 cho thấy Navisworks Manage 2026 nguyên bản không có trình nhập BCF và máy test chưa có plugin |
| C4 | **`ModelLinesFromCad`** — DWG đã link/import → model line cho `RouteFromLines`: lọc layer (wildcard), bỏ đoạn rác và đường vẽ chồng, nối đoạn thẳng hàng nhưng không xuyên ngã ba, ép về một cao độ | ✅ mã nguồn 2026-09-05: tầng thuần `Shared.Logic/Cad/CadCurveFilter` (23 ca test), lệnh Core, nút Ribbon, 2 ca kiểm trong `revit-mep`; chạy lại không sinh bản sao. ✅ **Đã chạy thật 2026-09-05 (§29)**: bộ ghi MEP nay tự dựng fixture bằng lệnh Core mới **`CadLink`** (link DWG/DXF vào view mặt bằng — bước Insert → Link CAD mà kỹ sư làm tay từng tầng, và batch đêm thì không ai bấm được), fixture là **DXF văn bản** đọc được trong repo nên không cần AutoCAD. Chuỗi 4 ca: link → link lại phải bỏ qua → dựng model line → chạy lại phải 0. Kết quả **`Đã tạo 2 model line`** — đúng con số fixture gài (hai đoạn thẳng hàng gộp thành 1 + đoạn dọc; đoạn rác 20 mm, layer `-TEXT`, layer `RAC` bị loại), lần hai 0 mới / 2 đã có. Vòng này lộ ra lỗi thật: với bản vẽ **link**, `ImportInstance.Name` không mang tên file (tên nằm ở `CADLinkType`), nên tham số **`dwgNameContains` chưa bao giờ khớp một bản vẽ link** — ai dùng nó đều nhận `E-PRECOND` và tưởng mô hình không có CAD; sửa ở `RevitCompat.CadFileName` dùng chung, và khi lọc không ra thì **in tên những bản vẽ đang có** thay vì bắt kỹ sư đoán. Đặc tả: [`dac-ta-tinh-nang.md`](dac-ta-tinh-nang.md) §3.0 |
