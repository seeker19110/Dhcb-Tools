...
| 9.3 🟡 | **Tiếng Việt hoá thông báo** | ✅ Đã dọn các chuỗi tiếng Anh còn sót mà vòng chạy thật 2026-09-03 in ra tận báo cáo: `[Dry Run] Would create 2 level(s).`, `[Skip]`, `[Create]`, `Family folder not found`, `Error: `, `[DryRun]` của BatchExport. `VietnameseMessageTests` (thuần, đọc mã nguồn Core) chốt chặn theo danh sách mẫu đã thấy tận mắt. ✅ Thêm mã lỗi `E-CONFIG-MISSING` (thiếu trường bắt buộc) cạnh `E-PARAM-MISSING`/`E-PATH-MISSING` sẵn có. ⬜ Còn: rà nốt phần `Messages` chi tiết của nhóm MEPF và gom bảng mã lỗi vào một trang tài liệu |
